  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="text-text-tertiary text-sm">규칙서 목록 불러오는 중...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center text-center py-20">
        ...
        <p className="text-danger font-medium mb-1">규칙서 목록을 불러올 수 없습니다</p>
        <p className="text-sm text-text-tertiary max-w-md">{error}</p>
      </div>
    );
  }
      ...
      {rulebooks.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {rulebooks.map((book) => {
            ...
            return (
              <div key={book.id} className="card p-5 hover:border-line-strong transition-all">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-text-primary truncate">
                      {name}
                    </h3>
                    <p className="text-sm text-text-tertiary mt-0.5">{system}</p>
                  </div>
                  <span
