...
export default function CharacterSheet() {
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      {conditions.length > 0 && (
        <div>
          <h4 className="text-xs font-medium text-slate-400 mb-2 uppercase tracking-wider">상태이상</h4>
          <div className="flex flex-wrap gap-1.5">
            {conditions.map((cond) => (
              <span
                key={cond}
                className="text-xs px-2 py-1 bg-purple-500/15 text-purple-400 rounded-lg border border-purple-500/20"
              >
      ...
      {/* 능력치 */}
      <div>
        <h4 className="text-xs font-medium text-slate-400 mb-2 uppercase tracking-wider flex items-center gap-1.5">
          <Sword size={12} />
          능력치
        </h4>
        <div className="grid grid-cols-3 gap-1.5">
          {(
            ...
            <div
              key={key}
              className="text-center p-2.5 bg-slate-700/20 rounded-xl border border-slate-700/30"
            >
              <div className="text-[10px] text-slate-500 mb-0.5 uppercase">{key}</div>
        ...
        <div>
          ...
          <ul className="space-y-1">
            {character.inventory.map((item, i) => (
              <li
                key={i}
                className="text-sm text-slate-400 flex items-center gap-2 py-0.5"
              >
