...
export async function createGameEngineForSession(
  ...
  let llmWrapper: { call: (messages: unknown[], tools: unknown[]) => Promise<GMResponse> } | null = null;
  ...
  if (keyRecords && keyRecords.length > 0) {
    ...
    for (const record of keyRecords) {
      try {
        const decryptedKey = decrypt(record.encrypted_key);
    ...
    if (apiKeyMap.size > 0) {
      ...
      llmWrapper = {
        call: async (messages: unknown[], tools: unknown[]): Promise<GMResponse> => {
          ...
          const toolDefs = (tools as Array<{ name: string; description: string; parameters: Record<string, unknown> }>).map(
            (t) => ({
              name: t.name,
